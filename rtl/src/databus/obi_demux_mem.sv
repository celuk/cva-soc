...
module obi_demux_mem (
   ...
   // verilog_format: off
   assign main_mem_addr_o = data_addr;
   assign uart_addr_o  = data_addr;
   assign timer_addr_o = data_addr;
   assign qspi_addr_o = data_addr;

   assign main_mem_wdata_o = data_wdata;
   assign uart_wdata_o  = data_wdata;
   assign timer_wdata_o = data_wdata;
   assign qspi_wdata_o = data_wdata;

   assign main_mem_req_o = (`MEM_BASE_ADDR  + `MEM_RANGE  > data_addr )   && (data_addr >= `MEM_BASE_ADDR)   ? (state == WAITING) & data_req : 'h0;
   assign main_mem_we_o  = (`MEM_BASE_ADDR  + `MEM_RANGE  > data_addr )   && (data_addr >= `MEM_BASE_ADDR)   ? data_we  : 'h0;
   assign main_mem_be_o  = (`MEM_BASE_ADDR  + `MEM_RANGE  > data_addr )   && (data_addr >= `MEM_BASE_ADDR)   ? data_be  : 'h0;
   ...
   assign data_rdata_o = (`MEM_BASE_ADDR+`MEM_RANGE     > data_addr) && (data_addr >= `MEM_BASE_ADDR  ) ? main_mem_rdata_i :
   ...
   assign periph_gnt   = (`MEM_BASE_ADDR+`MEM_RANGE     > data_addr) && (data_addr >= `MEM_BASE_ADDR  ) ? 1'b1        :
